...
class IncResBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1):
        ...
        self.conv1_2 = nn.Sequential(
            ...
            nn.BatchNorm2d(planes // 4),
            nn.ReLU(),
            nn.Conv2d(planes // 4, planes // 4, kernel_size=(1,3), stride=stride,  padding=(0,1), bias=False),
            nn.BatchNorm2d(planes // 4),
            nn.ReLU(),
            nn.Conv2d(planes // 4, planes // 4, kernel_size=(3,1), stride=stride, padding=(1,0), bias=False),
            nn.BatchNorm2d(planes // 4))
...
class Incrunet3(nn.Module):
    def __init__(self, in_ch, out_ch, args):
        super(Incrunet3, self).__init__()
        self.args = args
...
if __name__ == '__main__':
    ...
    # in_shape = (1,512,512)
    unet = Incrunet3(in_ch=1, out_ch=2).cuda()

    summary(unet, (1, 512, 512)) #Total params:  5,074,850
    # y = unet(x)
